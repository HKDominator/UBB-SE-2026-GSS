...
namespace Events_GSS.Views
{
    public sealed partial class MemoryView : UserControl
    {
        ...
        private void SortAscending_Click(object sender, RoutedEventArgs e)
        {
            ...
            ViewModel.ResetOnlyMineWithoutReload(); // setează _showOnlyMine = false fără load
            ViewModel.SortAscendingCommand.Execute(null); // singura operație care face load
        }
        ...
        private async void LikeButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                btn.IsChecked = memory.IsLikedByCurrentUser; // revert imediat, fără flash
            ...
            if (ViewModel.HasError)
                btn.IsChecked = memory.IsLikedByCurrentUser; // revert pentru orice altă eroare
        }
...
private void MyMemoriesToggle_Click(object sender, RoutedEventArgs e)
        ...
        private async void LikeButton_Click(object sender, RoutedEventArgs e)
        {
            if (sender is ToggleButton btn && btn.Tag is Memory memory)
            {
                await ViewModel.ToggleLikeAsync(memory);
        ...
        private async void LikeButton_Click(object sender, RoutedEventArgs e)
        {
            if (sender is ToggleButton btn && btn.Tag is int memoryId)
            {
                await ViewModel.ToggleLikeAsync(memoryId);
        ...
        private async void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            if (sender is not Button btn || btn.Tag is not int memoryId)
                return;
            ...
            var result = await dialog.ShowAsync();
            if (result == ContentDialogResult.Primary)
                await ViewModel.DeleteMemoryAsync(memory);
        }
        private async void DeleteButton_Click(object sender, RoutedEventArgs e)
        {
            if (sender is not Button btn || btn.Tag is not Memory memory)
                return;
